...
"""
    mat_fun(f, A; sep, max_deg, scale, color, ε, isnative)
...
function mat_fun(f::Function, A::AbstractMatrix{TT};               
    ...
    split_map = get_splittings(g, Λ; sep, max_deg, scale, color, ε, isnative)
    ...
    elseif allequal(split_map)
        ...
        F = block_parlett_recurrence(f, S.T, block; max_deg, isnative)
    end
